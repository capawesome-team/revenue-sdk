...
description: Try revenue-sdk with no account, no API keys, no fetch stubs and no test cards — a seedable in-memory provider from revenue-sdk/testing.
sidebar:
  label: Testing
---
...
The type is exported as `InMemoryUsageEvent`. Override `capabilities: { usageReporting: false }` to
...
:::note
A Dodo Payments test secret must be `whsec_` followed by valid base64 — its key derivation
base64-decodes the remainder, so anything else throws `RevenueError` with code `validation`. Polar uses
its secret verbatim and accepts any string.
:::
