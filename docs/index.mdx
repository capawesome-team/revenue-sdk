...
subscription, customer-portal, billing-history, and webhook logic once instead of five times.
...
<CardGroup cols={2}>
  ...
  <Card title="Normalized types" icon="shapes">
    ...
    untouched payload on a `raw` escape hatch. An `Order` is one payment, with a normalized `status`,
    readable from a webhook or from `orders.list`.
  </Card>
...
<CardGroup cols={3}>
  ...
  <Card title="Concepts" icon="book-open" href="/docs/concepts/client-and-providers">
    Clients, products, checkouts, subscriptions, orders, webhooks, pagination, errors, capabilities.
  </Card>
</CardGroup>
