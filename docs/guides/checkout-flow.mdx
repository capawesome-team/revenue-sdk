...
<Steps>
  <Step title="Create the checkout server-side">
    ...
    :::note
    Selling a pay-what-you-want product? Pass the amount the buyer picked as `customAmount`, in minor
    units, gated by the `checkoutCustomAmount` capability (Polar and Lemon Squeezy). Omit it and the
    buyer names the amount on the provider's page instead. See
