...
`amount` is `null` for every price model other than `fixed` — that is what the "Contact us" fallback
covers; see [Products & prices](/docs/concepts/products-and-prices).

Real catalogs also carry metered prices and one-off products you don't want on a pricing page, so filter
