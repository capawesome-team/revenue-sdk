...
replays — so there is nothing to derive. Store it with a TTL:
...
It works the same on every provider, including Lemon Squeezy, which publishes no event id and gets a
SHA-256 of the raw body instead — see
[where the key comes from](/docs/concepts/webhooks#where-the-key-comes-from).
...
:::warning
`idempotencyKey` dedupes **redeliveries of one event**. It does not collapse a provider's several events
for one state change — Polar and Dodo Payments emit both `subscription.active` and
`subscription.updated` for a single activation, two events with two distinct keys — and it cannot fix a
race inside your own handler. The next section is where that bites.
:::
...
:::warning
**The read-then-upsert above is a race, and event dedupe cannot save you from it.** `upsertSubscription`
reads the stored row, then writes in a second statement. Two deliveries handled concurrently can both
read the old row before either writes, so both pass the staleness guard, both see the same "change", and
any side effect hanging off that diff — a plan-changed email, a provisioning call, a credit grant — fires
twice. Their `idempotencyKey`s differ by construction, because they are two different events, so
