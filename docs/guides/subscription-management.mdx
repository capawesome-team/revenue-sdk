...
:::warning
On **Lemon Squeezy**, `PATCH /subscriptions` silently no-ops for PayPal-paid subscriptions. Re-read the
subscription and confirm `cancelAtPeriodEnd === false` before telling the customer it worked. The same
applies to pause and resume, which go through that endpoint too.
:::
