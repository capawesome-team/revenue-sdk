...
Out of scope (deliberate): key creation (Dodo-only upstream), issuance configuration (Polar benefit / LS variant setting / Dodo entitlement — all dashboard-configured), Polar's usage metering on keys (`increment_usage`, `conditions`, `limit_usage`), license-key webhook events. Testing provider: `InMemorySeed.licenseKeys?: InMemoryLicenseKeySeed[]` → `state.licenseKeys`; it deliberately does NOT implement the standalone public functions (they are subpath exports, not `RevenueProvider` members).
