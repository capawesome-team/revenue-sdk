...
Exactly three mappings read the PAYLOAD rather than the event string, and that is the complete list — do not add a fourth: the Polar license grant above, plus Polar `checkout.updated` and Stripe `checkout.session.completed`/`.async_payment_succeeded` (→ `checkout.completed` only when the mapped `Checkout.status` is `complete`). Payload-reading answers "did this reach a terminal state" and nothing else — never a `subscriptionChange`.
